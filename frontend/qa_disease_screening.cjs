...
async function run() {
  ...
  for (const viewport of [
    ...
    page.on("requestfailed", (req) => {
      failedRequests.push(
        `[${viewport.name}] ${req.method()} ${req.url()} — ${req.failure()?.errorText}`,
      );
    });
